...
static bool do_linux_lsort(int argc, char *argv[])
{
    if (argc != 1) {
        report(1, "%s takes no arguments", argv[0]);
        return false;
...
static void console_init()
{
    ...
    ADD_COMMAND(linux_lsort, "Sort queue in ascending order using list_sort",
                "");
    ADD_COMMAND(shuffle, "Shuffle queue by Fisher-Yates shuffle algorithm", "");
